...
public class window_max_min_temp {
    ...
    public static class WindowResult extends ProcessWindowFunction<Tuple2<Double, Double>, MaxMinTempPerWindow, String, TimeWindow> {
        @Override
        public void process(String s, Context context, Iterable<Tuple2<Double, Double>> iterable, Collector<MaxMinTempPerWindow> collector) throws Exception {
            Tuple2<Double, Double> tuple2 = iterable.iterator().next();
            collector.collect(new MaxMinTempPerWindow(s, tuple2.f0, tuple2.f1, context.window().getStart(), context.window().getEnd()));
        }
    }
